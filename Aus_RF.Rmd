...
for (i in 5:ncol(IncMSE_norm)){
  print(i)
  print(colnames(IncMSE)[i])
  print(paste0("min: ",min(IncMSE_norm[,i])))
  print(paste0("max: ",max(IncMSE_norm[,i])))
  min <- min(IncMSE_norm[,i])
  max <- max(IncMSE_norm[,i])
  range[i] <- max - min
  print(paste0("range: ",range[i]))
  print("")
  for (j in 1:nrow(IncMSE_norm)){
    print(paste0("cell ",j," before: ",IncMSE_norm[j,i]))
    IncMSE_norm[j,i] <- (IncMSE_norm[j,i]-min)/range[i]
    print(paste0("cell ",j," after: ",IncMSE_norm[j,i]))
  }
}
