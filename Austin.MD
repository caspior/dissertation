...
| Aggregate trips (dockless) | [PostGIS](Aus_counts.txt) | Austin's grid, Dockless cleaning output | Austin's grid with trip counts |
| Calculate land use proportions | [PostGIS](Aus_lu.txt) | Austin's grid with trip counts, [Austin's land uses](https://data.austintexas.gov/Locations-and-Maps/Land-Use-Inventory-Detailed/fj9m-h5qy) | Austin's grid with trips and land uses |
