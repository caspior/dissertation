...
dockless <- read.csv("Dockless_Vehicle_Trips 3-27.csv")
N <- nrow(dockless)
```

Generate dates:
```{r}
dockless$date <- as.character(dockless$Start.Time)
dockless$date <- substr(dockless$date, 1, 10)
dockless$date <- as.Date(dockless$date, "%m/%d/%Y")
```

Counting scooters and e-bike trips (before cleaning):
```{r}
